/**
 * guides.ts — the Guides tab's whole server-side domain, four concerns in
 * one small file: parsing a tutor deck's `<title>` and its `id="provenance"`
 * JSON stamp out of already-generated HTML; `scanGuides`, the recursive walk
 * over docs/guides/ that builds the GuidesIndex behind
 * GET /api/guides; `resolveGuidePath`, which turns a requested relPath into
 * a servable file for GET /guides/<relPath>; and `GUIDE_MIME`, the
 * content-type lookup for whatever it resolves to.
 *
 * Regex extraction (rather than a full HTML parser) is deliberate and safe
 * for the parsing half: every file under docs/guides/ is
 * machine-generated by the tutor/study skills — never adversarial HTML — so
 * there is no untrusted-input surface a real parser would need to defend
 * against.
 *
 * Two different safety contracts live here, deliberately. Parsing and
 * scanning fail open: a missing marker, missing stamp, malformed JSON, or an
 * unreadable file/directory yields null fields or an empty result, never a
 * throw — a broken deck should disappear from the tab, not take it down.
 * `resolveGuidePath` fails closed instead — it is the feature's entire
 * security surface, the only thing standing between an unauthenticated
 * request and the rest of the repository (docs/guides/ sits right
 * beside `.env`), so anything it cannot positively prove resolves strictly
 * inside `guidesDir` comes back `null` rather than a best guess. Read "fails
 * open" above as scoped to the parsing/scanning half only — applied to the
 * guard it would name the exact opposite of the behaviour it must have.
 */
...
async function scanDir(dir: string, relDir: string): Promise<WalkResult> {
  ...
  const results = await Promise.all(entries.map(async (entry): Promise<WalkResult> => {
    ...
    if (isRoot && entry.name === 'index.html') return EMPTY_WALK; // a root index.html is never a guide artifact
...
/**
 * Build the Guides tab's index by walking `guidesDir` (docs/guides/)
 ...
 *
 * This is the feature's entire security surface: docs/guides/
