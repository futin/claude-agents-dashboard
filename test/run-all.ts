/** Run every test module and exit nonzero if any fail. */
import { run as runTranscript } from './transcript.test.js';
import { run as runTitleCache } from './title-cache.test.js';
import { run as runScan } from './scan.test.js';
import { run as runArchived } from './archived.test.js';
import { run as runUsage } from './usage.test.js';
import { run as runAgents } from './agents.test.js';
import { run as runAgentsCache } from './agents-cache.test.js';
import { run as runChat } from './chat.test.js';
import { run as runAnalyze } from './analyze.test.js';
import { run as runMarkdown } from './markdown.test.js';
import { run as runChatFilter } from './chat-filter.test.js';
import { run as runFilterSort } from './filter-sort.test.js';
import { run as runTriage } from './triage.test.js';
import { run as runAnalyticsFilterSort } from './analytics-filter-sort.test.js';
import { run as runFrontmatter } from './frontmatter.test.js';
import { run as runManagement } from './management.test.js';
import { run as runManagementEntries } from './management-entries.test.js';
import { run as runSessionAnalyticsLog } from './sessionAnalyticsLog.test.js';
import { run as runAnalytics } from './analytics.test.js';
import { run as runPending } from './pending.test.js';
import { run as runPlans } from './plans.test.js';
import { run as runMessages } from './messages.test.js';
import { run as runPermissions } from './permissions.test.js';
import { run as runRemoteState } from './remoteState.test.js';
import { run as runOrigin } from './origin.test.js';
import { run as runSettings } from './settings.test.js';
import { run as runClientSettings } from './client-settings.test.js';
import { run as runNotify } from './notify.test.js';
import { run as runDeepLink } from './deep-link.test.js';
import { run as runBackClose } from './back-close.test.js';
import { run as runDictation } from './dictation.test.js';
import { run as runTranscribe } from './transcribe.test.js';
import { run as runApiBody } from './api-body.test.js';
import { run as runSpawn } from './spawn.test.js';
import { run as runSpawnEndpoint } from './spawn-endpoint.test.js';
import { run as runApiDismiss } from './api-dismiss.test.js';
import { run as runApiReadEndpoints } from './api-read-endpoints.test.js';
import { run as runApiWaitEndpoints } from './api-wait-endpoints.test.js';
import { run as runApiAnswerEndpoints } from './api-answer-endpoints.test.js';
import { run as runApiRemoteToggle } from './api-remote-toggle.test.js';
import { run as runApiRejectLog } from './api-reject-log.test.js';
import { run as runApiManagementAnalytics } from './api-management-analytics.test.js';
import { run as runSpawnOptions } from './spawn-options.test.js';
import { run as runResumeEligible } from './resume-eligible.test.js';
import { run as runSurface } from './surface.test.js';
import { run as runDecodePath } from './decode-path.test.js';
import { run as runStaticPath } from './static-path.test.js';
import { run as runFormat } from './format.test.js';
import { run as runPanelCollapse } from './panel-collapse.test.js';
import { run as runAgentLabel } from './agent-label.test.js';
import { run as runSplitPlan } from './split-plan.test.js';
import { run as runStopControl } from './stop-control.test.js';
import { run as runStickyStrip } from './sticky-strip.test.js';
import { run as runWebNotify } from './web-notify.test.js';
import { run as runWebNotifyClient } from './web-notify-client.test.js';
import { run as runStaleEnv } from './stale-env.test.js';
import { run as runUsagePace } from './usage-pace.test.js';
import { run as runPaceView } from './pace-view.test.js';
import { run as runUsageForecast } from './usage-forecast.test.js';
import { run as runUsageHistory } from './usage-history.test.js';
import { run as runUsageLedger } from './usage-ledger.test.js';
import { run as runUsageLedgerIo } from './usage-ledger-io.test.js';
import { run as runUsageRateClassify } from './usage-rate-classify.test.js';
import { run as runUsageRateDrift } from './usage-rate-drift.test.js';
import { run as runUsageRateFit } from './usage-rate-fit.test.js';
import { run as runUsageRateWeekly } from './usage-rate-weekly.test.js';
import { run as runApiUsageRates } from './api-usage-rates.test.js';
import { run as runAccount } from './account.test.js';
import { run as runApiAccount } from './api-account.test.js';
import { run as runUsageRateDaily } from './usage-rate-daily.test.js';
import { run as runUsageBoost } from './usage-boost.test.js';
import { run as runCheckTokenWeights } from './check-token-weights.test.js';
import { run as runRatesAudit } from './rates-audit.test.js';
import { run as runUsageRatesFormat } from './usage-rates-format.test.js';
import { run as runUsageProfileApi } from './usage-profile-api.test.js';
import { run as runUsageProfileView } from './usage-profile-view.test.js';
import { run as runWalkChart } from './walk-chart.test.js';
import { run as runWalkRows } from './walk-rows.test.js';
import { run as runTokenRefresh } from './token-refresh.test.js';
import { run as runDocsLinks } from './docs-links.test.js';
import { run as runEnvValue } from './env-value.test.js';
import { run as runInstallHooksToken } from './install-hooks-token.test.js';
import { run as runKillGuard } from './kill-guard.test.js';
import { run as runTailnet } from './tailnet.test.js';
import { run as runBreakpoints } from './breakpoints.test.js';
import { run as runChatPinnedPad } from './chat-pinned-pad.test.js';

let failed = 0;
failed += runTranscript();
failed += runTitleCache();
failed += runScan();
failed += runArchived();
failed += runUsage();
failed += runAccount();
failed += runUsagePace();
failed += runPaceView();
failed += runUsageForecast();
failed += runUsageHistory();
failed += runUsageLedger();
failed += runUsageLedgerIo();
failed += runUsageRateClassify();
failed += runUsageRateDrift();
failed += runUsageRateFit();
failed += runUsageRateWeekly();
failed += runCheckTokenWeights();
failed += runRatesAudit();
failed += runUsageRatesFormat();
failed += runUsageProfileApi();
failed += runUsageProfileView();
failed += runWalkChart();
failed += runWalkRows();
failed += await runTokenRefresh();
failed += runAgents();
failed += runAgentsCache();
failed += runChat();
failed += runAnalyze();
failed += runMarkdown();
failed += runChatFilter();
failed += runFilterSort();
failed += runTriage();
failed += runAnalyticsFilterSort();
failed += runFrontmatter();
failed += await runManagement();
failed += runManagementEntries();
failed += runSessionAnalyticsLog();
failed += runAnalytics();
failed += await runPending();
failed += await runPlans();
failed += await runMessages();
failed += await runPermissions();
failed += runRemoteState();
failed += runOrigin();
failed += runSettings();
failed += runClientSettings();
failed += await runNotify();
failed += runDeepLink();
failed += runBackClose();
failed += runDictation();
failed += await runTranscribe();
failed += await runApiBody();
failed += runSpawn();
failed += await runSpawnEndpoint();
failed += await runApiDismiss();
failed += await runApiReadEndpoints();
failed += await runApiWaitEndpoints();
failed += await runApiAnswerEndpoints();
failed += await runApiRemoteToggle();
failed += await runApiRejectLog();
failed += await runApiManagementAnalytics();
failed += await runApiUsageRates();
failed += await runApiAccount();
failed += runUsageRateDaily();
failed += runUsageBoost();
failed += runSpawnOptions();
failed += runResumeEligible();
failed += runSurface();
failed += runDecodePath();
failed += runStaticPath();
failed += runFormat();
failed += runPanelCollapse();
failed += runAgentLabel();
failed += runSplitPlan();
failed += runStopControl();
failed += runStickyStrip();
failed += runWebNotify();
failed += await runWebNotifyClient();
failed += await runStaleEnv();
failed += runDocsLinks();
failed += runEnvValue();
failed += runInstallHooksToken();
failed += runKillGuard();
failed += runTailnet();
failed += runBreakpoints();
failed += runChatPinnedPad();

console.log(failed > 0 ? `FAILED (${failed})` : 'ALL PASS');
process.exit(failed > 0 ? 1 : 0);
