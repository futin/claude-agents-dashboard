...
export function run(): number {
  ...
  if (test('relabelling gap intervals pre-ledger moves no fitted number at all', () => {
    ...
    // Both sides spread over dates, not minutes: the day floors added by
    // `bug-17` refuse a span-blind pool, and a `thin` row would satisfy the
    // equality below while proving nothing about a fitted number.
    const withGaps = [
      ...daily({ count: 40, days: 7, dUtil: 0.5, weightedPerPct: 900_000, rawPerPct: 9_000_000, model: 'A', endT: NOW - 4 * DAY }),
      ...daily({ count: 12, days: 2, dUtil: 0.5, weightedPerPct: 900_000, rawPerPct: 9_000_000, model: 'A', endT: NOW - MIN }),
