...
| `ANSWER_TOKEN` | _(empty)_ | Shared secret required by **every** write endpoint, not only the remote-answer ones: the [push](../subsystems/push-notify.md) routes the hooks POST to, [spawn](../subsystems/spawn.md), [dictation](../subsystems/dictation.md) and the settings write are all gated by it (`grep -c 'tokenOk(config, req)' server/api.ts`). Empty = open, matching the app's LAN-trust posture. Set it and the hooks need the same value in `~/.claude/hooks/dashboard-token`, or they are silently refused |
