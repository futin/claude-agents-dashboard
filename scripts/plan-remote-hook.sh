...
# How long the plan then stays answerable in the dashboard before we give up and
# let the plan card appear. Resolved exactly like the idle threshold — env var,
# then the dashboard's Settings page (same probe), then 600. Keep this hook's own
# `timeout` in ~/.claude/settings.json above this + 15, or the CLI kills the hook
# mid-wait (which just degrades to the terminal card).
