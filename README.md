...
- **Context bar + %** — current context tokens vs. the model's window (200k default, 1M for
  `[1m]` models). Turns orange/red as it fills.
...
**idle**. Fail-open — if `lsof` is unavailable the gate is skipped.
